...
- Give it a name like "notclicky-agent"
- Select scopes:
...
- Give it a title like "notclicky-agent-<machine-name>"
...
elif [ -f your shell environment or project `.env` ] && grep -q "^GITHUB_TOKEN=" your shell environment or project `.env`; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" your shell environment or project `.env` | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
