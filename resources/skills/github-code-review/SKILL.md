...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f your shell environment or project `.env` ] && grep -q "^GITHUB_TOKEN=" your shell environment or project `.env`; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" your shell environment or project `.env` | head -1 | cut -d= -f2 | tr -d '\n\r')
