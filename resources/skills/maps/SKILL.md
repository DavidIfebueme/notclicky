...
- Nominatim reverse: `https://nominatim.openstreetmap.org/reverse?format=json&lat=...&lon=...`
- OSRM route: `https://router.project-osrm.org/route/v1/driving/{lon1},{lat1};{lon2},{lat2}?overview=false`
...
```bash
curl -s "https://nominatim.openstreetmap.org/search?format=json&q=" # search "Eiffel Tower"
curl -s "https://nominatim.openstreetmap.org/search?format=json&q=" # search "1600 Pennsylvania Ave, Washington DC"
```
...
```bash
curl -s "https://nominatim.openstreetmap.org/reverse?format=json&lat=48.8584&lon=2.2945"
```
...
```bash
python3 maps_client.py search "Statue of Liberty"
# Should return lat ~40.689, lon ~-74.044

python3 maps_client.py nearby --near "Times Square" --category restaurant --limit 3
