---
name: notclicky-guided-tutorials
description: Use NotClicky's proper pointing tool calls, screenshots, captions, speech, and multi-call bridge to walk the user through visible macOS or app workflows step by step. Use when the user asks "how do I", "walk me through", "show me where", "tutorial", "teach me", or asks for guided help such as sending something with AirDrop.
version: 1.0.0
...
Use NotClicky's local external-control bridge as a real tool surface for guided tutorials. Do not fake pointing with hidden text tags when tool calls are available.
...
- `notclicky_point`: point NotClicky's native cursor at one screen coordinate with a short caption.
- `notclicky_point_many`: show several temporary secondary cursors at once.
...
- `speak`: speak a short instruction through NotClicky.
- `clear`: remove stale overlay markers.

Compatibility aliases still work: `show_cursor`, `show_cursors`, `notclicky_show_cursor`, and `notclicky_show_cursors`.
...
If you only have screenshot pixel coordinates, convert them into the screenshot's display frame before calling `notclicky_point`.
...
curl -s -X POST http://127.0.0.1:32123/mcp/call \
  ...
  -d '{"tool":"notclicky_point","arguments":{"x":820,"y":760,"caption":"Click Share","durationMs":4500}}'
```
...
curl -s -X POST http://127.0.0.1:32123/mcp/calls \
  ...
  -d '{
    "calls": [
      ...
      {"tool":"notclicky_point","arguments":{"x":820,"y":760,"caption":"Share menu","durationMs":3500}},
...
4. Call `notclicky_point` with a short caption, or do not point if the relevant target is missing or ambiguous.
...
- Use NotClicky's pointer for each visible next target rather than dumping all steps at once.
...
- Use `notclicky_point` as the normal pointing tool call.
- Use `notclicky_point_many` only when comparing or marking several choices.
- Use `speak` for short spoken prompts, not long lectures.
- Use `clear` before changing scenes.
- Do not click or type unless the user asked NotClicky to perform the action; this skill is for teaching and pointing.
...
Do not use coordinate clicks as tutorial steps unless the user asked NotClicky to perform the action rather than show it.
