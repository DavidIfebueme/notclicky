---
name: notclicky-screen-control
description: Instantly control NotClicky's local overlay bridge to point on screen, show captions, or speak through NotClicky's voice. Use when the user says "show me where you mean", "show me how to", "how do I do this", "point to it", "highlight that", "say this", or asks for visual on-screen guidance.
version: 1.2.0
...
Use NotClicky's local external-control bridge for immediate visual guidance. Do this instead of describing where to look when the user asks you to show them.
...
The bridge is designed to be fast and non-invasive: commands only drive NotClicky's proxy overlay/voice layer and do not start dictation, submit prompts, create agent sessions, or mutate the main app conversation.

Important cursor model:
- NotClicky's cursor is the little triangle that normally follows the user's real system pointer.
- Default `/cursor` uses NotClicky's native smooth pointing choreography: the triangle zips to the target, captions it, then flies back. It does not warp the real macOS pointer and does not draw a duplicate primary cursor icon.
...
Optional fields:
- `caption`: short text shown beside NotClicky's primary cursor
- `durationMs`: how long to keep the caption visible; default is about 4s
- `travelMs`: accepted for compatibility, but primary cursor motion uses NotClicky's native smooth pointing choreography.
...
Use this when showing multiple possible locations or comparing alternatives. These are additional colored NotClicky-style cursors, not the user's primary pointer.
...
If `x`/`y` are omitted, NotClicky shows the caption near the current mouse location.
...
If NotClicky is already speaking, this returns HTTP 409 unless `interrupt: true` is passed. Prefer not to interrupt unless the user explicitly wants the new instruction spoken now.
...
curl -s -X POST http://127.0.0.1:32123/mcp/call \
  -H 'Content-Type: application/json' \
  -d '{"tool":"notclicky_point","arguments":{"x":640,"y":520,"caption":"This one"}}'
```

Supported tool names:
- `notclicky_point` (preferred single-target pointing tool)
- `notclicky_point_many` (preferred multi-marker pointing tool)
- `show_cursor` / `notclicky_show_cursor` (compatibility aliases)
- `show_cursors` / `notclicky_show_cursors` (compatibility aliases)
- `show_caption`
- `screenshot`
- `speak`
- `clear`
...
curl -s -X POST http://127.0.0.1:32123/mcp/calls \
  -H 'Content-Type: application/json' \
  -d '{"calls":[{"tool":"clear","arguments":{}},{"tool":"notclicky_point","arguments":{"x":640,"y":520,"caption":"Start here"}},{"tool":"speak","arguments":{"text":"Start with this button."}}]}'
```
...
curl -s -X POST http://127.0.0.1:32123/mcp \
  -H 'Content-Type: application/json' \
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"notclicky_point","arguments":{"x":640,"y":520,"caption":"This one"}}}'
```
...
2. If you already know that relevant coordinate, immediately call `notclicky_point` or `/cursor` with the best available coordinate.
...
curl -s -X POST http://127.0.0.1:32123/scribble \
  -H "Authorization: Bearer $NOTCLICKY_BRIDGE_TOKEN" \
  -H 'Content-Type: application/json' \
