...
Use NotClicky's local external-control bridge to create a visual tour directly on the user's screen.
...
- Use default `POST /cursor` for the primary NotClicky pointer choreography. This uses NotClicky's existing smooth point-and-return animation and must not warp the real macOS pointer.
...
'''
    path = '/tmp/notclicky-tour-screen.swift'
