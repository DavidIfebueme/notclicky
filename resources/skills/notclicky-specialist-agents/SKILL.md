---
name: notclicky-specialist-agents
description: Create, equip, or repair NotClicky specialist agents with explicit soul, instructions, memory, heartbeat, and skills.json wiring.
---
...
# NotClicky Specialist Agents

Use this skill when the user asks NotClicky to create a new specialist, expert agent, recurring maintenance agent, or agent with domain expertise.

## Workflow

1. Inspect existing agents before writing:
   - Built-ins: `~/Library/Application Support/NotClicky/agents/`
   - User agents: `~/.notclicky/agents/`
   - Runtime map: `~/Library/Application Support/NotClicky/AgentMode/CodexHome/NotClickyRuntimeMap.md`
...
4. Keep the specialist bounded:
   ...
   - Include archive-first rules for NotClicky memory, logs, skills, prompts, config, and other durable artifacts.
...
```json
{
  "enabledSkillIDs": ["notclicky-specialist-agents"]
}
```
