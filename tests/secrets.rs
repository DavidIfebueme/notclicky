...
fn env_var_overrides_file() {
    unsafe { std::env::set_var("ZAI_API_KEY", "from-env-override-test"); }
    let secrets = Secrets::load().unwrap();
    assert_eq!(secrets.get("ZAI_API_KEY").unwrap(), "from-env-override-test");
...
fn require_missing_key_returns_error() {
    unsafe { std::env::remove_var("ZAI_API_KEY"); }
    let secrets = Secrets::load().unwrap();
    if secrets.get("ZAI_API_KEY").is_none() {
        let err = secrets.require("ZAI_API_KEY").unwrap_err();
...
fn save_and_reread_secrets() {
    ...
    secrets.set("TEST_KEY_SAVE", "test-value-42").ok();

    let path = config_dir.join("secrets.env");
    if path.exists() {
        let content = fs::read_to_string(&path).unwrap();
        assert!(content.contains("TEST_KEY_SAVE=test-value-42"));
